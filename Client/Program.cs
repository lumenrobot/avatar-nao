...
namespace Client
{
    class Program
    {
        ...
        static void putar()
        {
            ...
            properties.ReplyTo = "";
            Command com = new Command() { type = "motion", method = "moveto", parameter = new Parameter { x = 0.3f, y = 0.0f, tetha = 0.0f } };
            string body = JsonConvert.SerializeObject(com);
            byte[] buffer = Encoding.UTF8.GetBytes(body);
            channel.BasicPublish("amq.topic", "avatar.NAO.command", properties, buffer);
        ...
        static void testAck()
        {
            ...
            channel.QueueBind(queue, "amq.topic", queue);
            QueueingBasicConsumer consumer = new QueueingBasicConsumer(channel);
            channel.BasicConsume(queue, true, consumer);
            var properties = channel.CreateBasicProperties();
            string corId = Guid.NewGuid().ToString();
            properties.CorrelationId = corId;
            properties.ReplyTo = queue;
            Command com = new Command { type = "posture", method = "goToPosture", parameter = new Parameter {postureName = "Stand",speed=0.5f} };
            string body = JsonConvert.SerializeObject(com);
            byte[] buffer = Encoding.UTF8.GetBytes(body);
            channel.BasicPublish("amq.topic", channelKey, properties, buffer);
